...
#if defined(NDEBUG)
# error "Vermo cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Vermo Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees, int nHeight)
{
    ...
    if(nHeight <= 50) // Anti-Instamine rewards 1
    {
	nSubsidy = 1 * COIN;
    }
    else if(nHeight <= 4370) // 437000 coins / 3 days
    {
	nSubsidy = 100 * COIN;
    }
    else	// after 3 days Rewards 1 forever
    {
	nSubsidy = 1 * COIN;
    }
...
{
    int64_t nSubsidy;
	if(nHeight >= 4310) // random PoS reward Starts after 3 days of launch
	{
    int rand = 0;
    std::string cseed_str = prevHash.ToString().substr(7,7);
    const char* cseed = cseed_str.c_str();
    long seed = hex2long(cseed);
    rand = generateMTRandom(seed, 99); 
    nSubsidy = (1 + rand) * COIN; // random PoS reward from 1 to 100
	}
	else if(nHeight >= 10080) // random PoS reward ends after 7 days 
    {
	nSubsidy = 1 * COIN;
    }
...
static const int64_t nTargetTimespan = 2 * 60;  // 2 min Retarget 
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (TestNet())
    {
        nStakeMinAge = 30 * 60; // test net min age is 30 min
